...
{
public:
   Book(string bname = "MyBook", string bauthor = "Darcy", double bprice = 20.0, unsigned int bnumber = 100);
...
void Book::display()
{
   cout << "BookName: " << bookName << '\t' << "Author: " << author << '\t' << "Price: " << price << '\t' << "Number: " << number << endl;
   return;
}
...
int main()
{
   ...
   book.setname("C++程序设计");
   char temp;
   cout << "click b to borrow, r to restore, s to display and e to exit(b/r/s/e)";
